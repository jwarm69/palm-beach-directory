...
import { getOfferIconProps } from "@/lib/utils";
...
export function OfferIcon({ type, isFlash = false, className }: OfferIconProps) {
  const { className: sizeClass, strokeWidth, path } = getOfferIconProps(type, isFlash);
...
export function HeartIcon({ className = "w-4 h-4", filled = false }: { className?: string; filled?: boolean }) {
  if (filled) {
    return (
